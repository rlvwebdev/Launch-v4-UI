...
export const Header = ({ className = '', onThemeChange }: HeaderProps) => {
  ...
  // Trigger rocket return animation on component mount
  useEffect(() => {
    const wasLaunched = sessionStorage.getItem('rocketLaunched');
    if (wasLaunched === 'true') {
      // Small delay to ensure DOM is ready
      requestAnimationFrame(() => {
        setRocketReturning(true);
        sessionStorage.removeItem('rocketLaunched');
      });
      const timer = setTimeout(() => setRocketReturning(false), 1000);
    ...
    <>
      ...
      <header className={`header ${className}`}>
        <div className="header__left">
          <Link 
            to="/" 
            className={`header__logo-link ${rocketLaunching ? 'header__logo-link--launching' : ''} ${rocketReturning ? 'header__logo-link--returning' : ''}`}
            onClick={() => {
              setRocketLaunching(true);
              sessionStorage.setItem('rocketLaunched', 'true');
              setTimeout(() => setRocketLaunching(false), 1000);
            }}
          >
        ...
        <div className="header__right">
          <nav className="header__nav">
            <div className="header__menu-container" ref={accountMenuRef}>
              ...
              <div className={`header__dropdown ${accountMenuOpen ? 'header__dropdown--open' : ''}`}>
                <Link to="/account/profile" className="header__dropdown-item" onClick={() => setAccountMenuOpen(false)}>
                ...
                <Link to="/account/settings" className="header__dropdown-item" onClick={() => setAccountMenuOpen(false)}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
            ...
            <div className="header__menu-container" ref={helpMenuRef}>
              ...
              <div className={`header__dropdown ${helpMenuOpen ? 'header__dropdown--open' : ''}`}>
                <Link to="/help/documentation" className="header__dropdown-item" onClick={() => setHelpMenuOpen(false)}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
                ...
                <Link to="/help/support" className="header__dropdown-item" onClick={() => setHelpMenuOpen(false)}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
                    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                  </svg>
                  <span>Support</span>
                </Link>
                <Link to="/help/feedback" className="header__dropdown-item" onClick={() => setHelpMenuOpen(false)}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
          ...
          <div className="header__mobile-menu-container" ref={mobileMenuRef}>
            <button
        ...
        <Link to="/account/profile" className="header__mobile-item" onClick={() => setMobileMenuOpen(false)}>
        ...
        <Link to="/account/settings" className="header__mobile-item" onClick={() => setMobileMenuOpen(false)}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18" style={{ display: 'inline-block', marginRight: '0.5rem' }}>
            <circle cx="12" cy="12" r="3" />
            <path d="M12 1v6m0 6v6M4.93 4.93l4.24 4.24m5.66 5.66l4.24 4.24M1 12h6m6 0h6M4.93 19.07l4.24-4.24m5.66-5.66l4.24-4.24" />
          </svg>
        ...
        <Link to="/help/documentation" className="header__mobile-item" onClick={() => setMobileMenuOpen(false)}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18" style={{ display: 'inline-block', marginRight: '0.5rem' }}>
            <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20" />
            <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z" />
          </svg>
        ...
        <Link to="/help/support" className="header__mobile-item" onClick={() => setMobileMenuOpen(false)}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18" style={{ display: 'inline-block', marginRight: '0.5rem' }}>
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
          </svg>
          Support
        </Link>
        <button className="header__mobile-item" onClick={cycleTheme}>
          {getThemeIcon()}
          <span style={{ marginLeft: '0.5rem' }}>Theme: {currentTheme}</span>
        </button>
